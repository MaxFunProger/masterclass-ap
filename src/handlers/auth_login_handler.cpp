...
std::string AuthLoginHandler::HandleRequestThrow(
  ...
  if (!payload.HasMember("phone") || !payload.HasMember("password")) {
      throw userver::server::handlers::ClientError(
        userver::server::handlers::ExternalBody{"missing phone or password"});
  }
  ...
  auto result =
      db_cluster_->Execute(ClusterHostType::kSlave, sql::kSelectUserByPhone, *phone_digits);

  if (result.IsEmpty()) {
      request.SetResponseStatus(userver::server::http::HttpStatus::kUnauthorized);
      return userver::formats::json::ToString(
          userver::formats::json::MakeObject("status", "error", "message", "User not found"));
  }
  ...
  if (input_hash != stored_hash) {
      request.SetResponseStatus(userver::server::http::HttpStatus::kUnauthorized);
      return userver::formats::json::ToString(
          userver::formats::json::MakeObject("status", "error", "message", "Invalid password"));
  }
  ...
  response["telegram_nick"] = row["telegram_nick"].As<std::optional<std::string>>().value_or("");
