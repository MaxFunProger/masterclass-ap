...
std::string McListHandler::HandleRequestThrow(
  ...
  if (request.HasArg("min_age")) min_age = std::stoi(request.GetArg("min_age"));

  std::optional<double> max_price;
  if (request.HasArg("max_price")) max_price = std::stod(request.GetArg("max_price"));
  
  std::optional<double> min_price;
  if (request.HasArg("min_price")) min_price = std::stod(request.GetArg("min_price"));

  std::optional<double> min_rating;
  if (request.HasArg("min_rating")) min_rating = std::stod(request.GetArg("min_rating"));

  std::optional<std::string> category_opt = category.empty() ? std::nullopt : std::optional<std::string>(category);
  ...
  std::optional<std::string> audience_opt = audience.empty() ? std::nullopt : std::optional<std::string>(audience);
  std::optional<std::string> tags_opt = tags.empty() ? std::nullopt : std::optional<std::string>(tags);
  std::optional<std::string> format_opt = format.empty() ? std::nullopt : std::optional<std::string>(format);
  std::optional<std::string> company_opt = company.empty() ? std::nullopt : std::optional<std::string>(company);
  ...
  const userver::storages::postgres::Query* query_ptr = &sql::kSelectMasterclassesFiltered;
  ...
  const auto result = db_cluster_->Execute(
      ClusterHostType::kSlave, *query_ptr, 
      category_opt, audience_opt, tags_opt, format_opt, company_opt,
      min_age, max_price, min_price, min_rating, exclude_ids_opt,
      event_date_from_opt, event_date_to_opt, limit, offset);
  ...
  for (const auto& row : result) {
    ...
    entry["format"] = row["format"].As<std::optional<std::string>>().value_or("offline");
    entry["company"] = row["company"].As<std::optional<std::string>>().value_or("single");
    entry["category"] = row["category"].As<std::optional<std::string>>().value_or("");
    entry["min_age"] = row["min_age"].As<std::optional<int>>().value_or(0);
    entry["rating"] = row["rating"].As<std::optional<double>>().value_or(5.0);
